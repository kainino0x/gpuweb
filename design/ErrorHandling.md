...
## Open Questions and Considerations
 ...
 - Or should error scope balance be enforced by changing the API to `device.withErrorScope(filter, callback)`?
...
## Resolved Questions
 ...
 - Should there be a way to capture out-of-memory errors without capturing validation errors?
    - Resolved: No, applications (and conformance tests) can distinguish them by `error.type`.
