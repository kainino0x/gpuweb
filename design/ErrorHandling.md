...
An app should never give up on getting WebGPU access due to a `requestDevice` rejection
or a `GPUDevice.lost`.
It should only do give up based on a `requestAdapter` rejection.
...
A page begins loading in a tab, but then the tab is backgrounded.
 - On load, the page attempts creation of an adapter.
    - When the page is foregrounded, the `requestAdapter` Promise will resolve.
